...
Elm.TIDY = Elm.TIDY || {};
Elm.TIDY.RawTypes = Elm.TIDY.RawTypes || {};
Elm.TIDY.RawTypes.make = function (_elm) {
   "use strict";
   _elm.TIDY = _elm.TIDY || {};
   _elm.TIDY.RawTypes = _elm.TIDY.RawTypes || {};
   if (_elm.TIDY.RawTypes.values) return _elm.TIDY.RawTypes.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Array = Elm.Array.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var ClusterFile = F6(function (a,b,c,d,e,f) {
   ...
   var Cluster = F7(function (a,b,c,d,e,f,g) {
      return {edit: a
             ,choices: b
             ,value: c
             ,size: d
   ...
   var ClusterChoice = F2(function (a,b) {
      return {v: a,c: b};
   });
   var decodeCluster = A8($Json$Decode.object7,
   Cluster,
   A2($Json$Decode._op[":="],"edit",$Json$Decode.bool),
   A2($Json$Decode._op[":="],
   "choices",
   $Json$Decode.array(A3($Json$Decode.object2,
   ClusterChoice,
   A2($Json$Decode._op[":="],"v",$Json$Decode.string),
   A2($Json$Decode._op[":="],"c",$Json$Decode.$int)))),
   A2($Json$Decode._op[":="],"value",$Json$Decode.string),
   A2($Json$Decode._op[":="],"size",$Json$Decode.$int),
   A2($Json$Decode._op[":="],"rowCount",$Json$Decode.$int),
   A2($Json$Decode._op[":="],"avg",$Json$Decode.$float),
   A2($Json$Decode._op[":="],"variance",$Json$Decode.$float));
   var decodeClusterFile = A7($Json$Decode.object6,
   ClusterFile,
   A2($Json$Decode._op[":="],"projectName",$Json$Decode.string),
   A2($Json$Decode._op[":="],"columnName",$Json$Decode.string),
   A2($Json$Decode._op[":="],"timeStamp",$Json$Decode.string),
   A2($Json$Decode._op[":="],"clusterMethod",$Json$Decode.string),
   A2($Json$Decode._op[":="],"keyingFunction",$Json$Decode.string),
   A2($Json$Decode._op[":="],
   "clusters",
   $Json$Decode.array(decodeCluster)));
   return _elm.TIDY.RawTypes.values = {_op: _op
                                      ,ClusterChoice: ClusterChoice
                                      ,Cluster: Cluster
                                      ,ClusterFile: ClusterFile
                                      ,decodeCluster: decodeCluster
                                      ,decodeClusterFile: decodeClusterFile};
};
Elm.TIDY = Elm.TIDY || {};
Elm.TIDY.make = function (_elm) {
   "use strict";
   _elm.TIDY = _elm.TIDY || {};
   if (_elm.TIDY.values) return _elm.TIDY.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Array = Elm.Array.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   ...
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm),
   $TIDY$RawTypes = Elm.TIDY.RawTypes.make(_elm),
   $Task = Elm.Task.make(_elm);
   var _op = {};
   var btnClass = function (x) {
      return $Html$Attributes.$class(A2($Basics._op["++"],
      "btn btn-",
      A2($Basics._op["++"],x," btn-lg btn-block")));
   };
   var updateIndex = F2(function (model,i) {
      var propV = A2($Maybe.withDefault,
      "",
      A2($Maybe.map,
      function (x) {
         return x.value;
      },
      A2($Array.get,i,model.clusters)));
      return _U.update(model,
      {currentIndex: _U.cmp(i,0) < 0 ? 0 : i,proposedValue: propV});
   });
   var update = F2(function (action,model) {
      var _p0 = action;
      switch (_p0.ctor)
      {case "LoadClusters": var _p1 = _p0._0;
           if (_p1.ctor === "Just") {
                 return {ctor: "_Tuple2"
                        ,_0: _U.update(model,
                        {clusters: A2($Array.map,
                        function (x) {
                           return _U.update(x,{value: ""});
                        },
                        _p1._0.clusters)})
                        ,_1: $Effects.none};
              } else {
         ...
         case "PreviousCluster": return {ctor: "_Tuple2"
                                        ,_0: A2(updateIndex,model,model.currentIndex - 1)
                                        ,_1: $Effects.none};
         case "NextCluster": return {ctor: "_Tuple2"
                                    ,_0: A2(updateIndex,model,model.currentIndex + 1)
                                    ,_1: $Effects.none};
         case "StartFromBeginning": return {ctor: "_Tuple2"
                                           ,_0: A2(updateIndex,model,0)
                                           ,_1: $Effects.none};
         case "AcceptCluster": var accepted = _U.update(_p0._0,
           {edit: true,value: model.proposedValue});
           var updatedClusters = A3($Array.set,
           model.currentIndex,
           accepted,
           model.clusters);
           var model0 = _U.update(model,{clusters: updatedClusters});
           return {ctor: "_Tuple2"
                  ,_0: A2(updateIndex,model0,model0.currentIndex + 1)
                  ,_1: $Effects.none};
         default: return {ctor: "_Tuple2"
                         ,_0: _U.update(model,{proposedValue: _p0._0})
                         ,_1: $Effects.none};}
   });
   var initModel = {clusters: $Array.empty
                   ,currentIndex: 0
                   ,proposedValue: ""};
   var SetProposedString = function (a) {
      return {ctor: "SetProposedString",_0: a};
   };
   var viewClusterChoice = F3(function (address,model,cc) {
      var selected = _U.eq(cc.v,model.proposedValue);
      return A2($Html.div,
      _U.list([A2($Html$Events.onClick,
              address,
              SetProposedString(cc.v))
              ,$Html$Attributes.$class(selected ? "bg-success" : "")]),
      _U.list([A2($Html.h2,
      _U.list([$Html$Attributes.$class("p-y")]),
      _U.list([$Html.text(cc.v)
              ,A2($Html.small,
              _U.list([$Html$Attributes.$class("text-muted")]),
              _U.list([$Html.text(A2($Basics._op["++"],
              " (",
              A2($Basics._op["++"],$Basics.toString(cc.c),")")))]))]))]));
   });
   var viewCluster = F3(function (address,model,cluster) {
      return A2($Html.div,
      _U.list([]),
      $Array.toList(A2($Array.map,
      A2(viewClusterChoice,address,model),
      cluster.choices)));
   });
   var stringInput = F2(function (address,string) {
      return A2($Html.input,
      _U.list([$Html$Attributes.placeholder("Corrected value")
              ...
              function (_p2) {
                 return A2($Signal.message,address,SetProposedString(_p2));
              })]),
      _U.list([]));
   });
   var StartFromBeginning = {ctor: "StartFromBeginning"};
   var NextCluster = {ctor: "NextCluster"};
   var PreviousCluster = {ctor: "PreviousCluster"};
   var AcceptCluster = function (a) {
      return {ctor: "AcceptCluster",_0: a};
   };
   var view = F2(function (address,model) {
      var canAccept = !_U.eq(model.proposedValue,"");
      var maybeCluster = A2($Array.get,
      model.currentIndex,
      model.clusters);
      var accept = A2($Maybe.withDefault,
      StartFromBeginning,
      A2($Maybe.map,AcceptCluster,maybeCluster));
      return A2($Html.div,
      ...
      _U.list([A2($Html.div,
              ...
              _U.list([A2($Html.h1,
                      _U.list([$Html$Attributes.$class("display-1 text-muted")]),
                      _U.list([$Html.text("Select the correct one:")]))
                      ,A2($Maybe.withDefault,
                      A2($Html.div,_U.list([]),_U.list([])),
                      A2($Maybe.map,A2(viewCluster,address,model),maybeCluster))]))]))
              ,A2($Html.div,
              _U.list([$Html$Attributes.$class("row")]),
              _U.list([A2($Html.div,
              _U.list([$Html$Attributes.$class("col-xs-12")]),
              _U.list([A2(stringInput,address,model.proposedValue)]))]))
              ,A2($Html.div,
              _U.list([$Html$Attributes.$class("row")]),
              _U.list([A2($Html.div,
                      _U.list([$Html$Attributes.$class("col-xs-4")]),
                      _U.list([A2($Html.button,
                      _U.list([A2($Html$Events.onClick,address,PreviousCluster)
                              ,btnClass("secondary")
                      ...
                      _U.list([$Html$Attributes.$class("col-xs-4")]),
                      _U.list([A2($Html.button,
                      _U.list([A2($Html$Events.onClick,address,NextCluster)
                              ,btnClass("warning")
                              ,$Html$Attributes.disabled(false)]),
                      _U.list([$Html.text("Skip")]))]))
                      ,A2($Html.div,
                      _U.list([$Html$Attributes.$class("col-xs-4")]),
                      _U.list([A2($Html.button,
                      _U.list([A2($Html$Events.onClick,address,accept)
                              ,btnClass(canAccept ? "success" : "secondary")
                              ,$Html$Attributes.disabled($Basics.not(canAccept))]),
                      _U.list([$Html.text("Accept")]))]))]))]));
   });
   var LoadClusters = function (a) {
      return {ctor: "LoadClusters",_0: a};
   };
   var initEffects = $Effects.task(A2($Task.map,
   LoadClusters,
   $Task.toMaybe(A2($Http.get,
   $TIDY$RawTypes.decodeClusterFile,
   A2($Http.url,"clusters_sample.json",_U.list([]))))));
   var init = {ctor: "_Tuple2",_0: initModel,_1: initEffects};
   var Model = F3(function (a,b,c) {
      return {clusters: a,currentIndex: b,proposedValue: c};
   });
   return _elm.TIDY.values = {_op: _op
                             ,Model: Model
                             ,LoadClusters: LoadClusters
                             ,AcceptCluster: AcceptCluster
                             ,PreviousCluster: PreviousCluster
                             ,NextCluster: NextCluster
                             ,StartFromBeginning: StartFromBeginning
                             ,SetProposedString: SetProposedString
                             ,initModel: initModel
                             ,initEffects: initEffects
                             ,init: init
                             ,updateIndex: updateIndex
                             ,update: update
                             ,viewClusterChoice: viewClusterChoice
...
Elm.Main.make = function (_elm) {
   ...
   $TIDY = Elm.TIDY.make(_elm),
   $Task = Elm.Task.make(_elm);
   var _op = {};
   var app = $StartApp.start({init: $TIDY.init
                             ,view: $TIDY.view
                             ,update: $TIDY.update
                             ,inputs: _U.list([])});
   var main = app.html;
